...
\begin{frame}{Example}
  ...
  \only<2-4>{
    \begin{eqnarray*}
      \onslide<2-4>{\ddx(x^2e^{2x})}\onslide<3-4>{ &=& 2xe^{2x} + x^2(2e^{2x})\\} \onslide<4>{&=& 2xe^{2x}(1 + x)}
    \end{eqnarray*}
  }
  \only<5-7>{
    \begin{eqnarray*}
      \onslide<5-7>{\frac{\operatorname{d}}{\operatorname{dt}}(t^3\ln(t+1))}\onslide<6-7>{ &=& 3t^2\ln(t+1) + t^3\left(\frac{1}{t+1}\right)}\\
      \onslide<7>{&=& 3t^2\ln(t+1) + \frac{t^3}{t+1}}.
    \end{eqnarray*}
  }
  \only<8->{
    \begin{eqnarray*}
      \onslide<8->{\ddx(3x^2 + 5x)e^x}\onslide<9->{&=& (6x + 5)e^x + (3x^2 + 5x)e^x\\}
...
\begin{frame}{Example}
  ...
  \onslide<7->{
    \begin{eqnarray*}
      \onslide<7->{\frac{\operatorname{d}}{\operatorname{dq}}R(q) &=& \frac{\operatorname{d}}{\operatorname{dq}}(80qe^{-0.003q})\\}
      \onslide<8->{&=& 80\frac{\operatorname{d}}{\operatorname{dq}}qe^{-0.003q}\\}
...
\begin{frame}{Example}
  ...
  \begin{overprint}
    \only<2-5>{
      \begin{eqnarray*}
        \onslide<2-5>{\ddx\left(\frac{5x^2}{x^3 + 1}\right)}\onslide<3-5>{ &=& \frac{10(x^3 + 1) - 5x^2(3x^2)}{(x^3 + 1)^2}\\}
    ...
    \only<6-8>{
      \begin{eqnarray*}
        \onslide<6-8>{\ddx\frac{1}{1 + e^x}} \onslide<7-8>{&=& \frac{0(1 + e^x) - (1)e^x}{(1 + e^x)^2}\\}
        \onslide<8>{&=& \frac{-e^x}{(1 + e^x)^2}.}
      \end{eqnarray*}
      }
    \only<9->{
      \begin{eqnarray*}
        \onslide<9->{\ddx\frac{e^x}{x^2}} \onslide<10->{&=& \frac{e^xx^2 - e^x(2x)}{x^4}\\}
